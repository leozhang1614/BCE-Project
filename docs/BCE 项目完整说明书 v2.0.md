...
| 功能 | 说明 | 预计工时 | v2.0 更新 |
|------|------|---------|---------|
| 定时轮询服务 | 每 5 分钟扫描任务，飞书提醒 | 1 小时 | +频次控制 |
...
if (task.reminderCount >= MAX_REMINDERS) {
  console.log(`[轮询] 任务${task.id}已达提醒上限，跳过`);
  return;
}
